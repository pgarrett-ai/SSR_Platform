...
def test_harvest_sd_events_includes_lopucki_brd(tmp_path, monkeypatch, capsys):
    csv_path = tmp_path / "brd.csv"
    csv_path.write_text(
        "NameCorp,CikBefore,DateFiled\n"
        '"Alpha Aviation, Inc.",0000012345,3/25/2009\n'
        '"Alpha Aviation, Inc.",0000012345,1/1/2015\n'   # dup obligor, later date -> dropped
        '"Beta Retail Corp",,1/15/2019\n'                # blank CIK, name resolvable
        '"Gamma Foreign GmbH",,2/2/2018\n',              # blank CIK, name unresolvable
        encoding="utf-8",
    )
    ...
    events = labels.harvest_sd_events()

    # zero-padded CikBefore normalized to the same lstrip("0") format the 8-K harvest uses
    assert [(e["cik"], e["filed"], e["source"]) for e in events] == [
        ("12345", "2009-03-25", "lopucki_brd"),   # M/D/YYYY -> ISO; earliest of the two dup rows
        ("777", "2019-01-15", "lopucki_brd"),     # blank CikBefore resolved via name lookup
    ]
